...
const vertices = radarCapabilities.map(({ score }, index) => {
...
export function AboutX1Page() {
  ...
  return <article className="executive-about">
    ...
    <section className="about-section advisory-journey" aria-labelledby="journey-title"><p className="academic-eyebrow">Cybersecurity Advisory Journey</p><h2 id="journey-title">From Business Context to Resilient Outcomes</h2><p className="section-copy">A connected journey that brings business context, leadership choices and practical action into one view.</p>
      <div className="roadmap" role="tablist" aria-label="Cybersecurity advisory journey">
        ...
        {roadmap.map((item, index) => <button id={`stage-${item.id}`} key={item.id} type="button" role="tab" aria-selected={stage === index} aria-current={stage === index ? 'step' : undefined} aria-controls="journey-detail" tabIndex={stage === index ? 0 : -1} className={stage === index ? 'is-active' : ''} onClick={() => setStage(index)} onKeyDown={(event) => selectWithArrows(event, index, roadmap.length, setStage)}><span className="milestone-copy"><b>{String(index + 1).padStart(2, '0')}</b><strong>{item.stage}</strong><small>{item.shortLabel}</small></span><i className="milestone-node" aria-hidden="true" /></button>)}
      </div>
