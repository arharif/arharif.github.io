import { seedContent, seedTopics } from '@/content/data';
import { ContentInput, ContentRecord, ContentStatus, TopicInput, TopicRecord } from '@/content/types';
...
const normalizeTopic = (r: Record<string, unknown>): TopicRecord => ({
  ...
  icon: r.icon ? String(r.icon) : undefined, orderIndex: Number(r.order_index ?? 0), createdAt: String(r.created_at), updatedAt: String(r.updated_at),
});
...
const normalizeContent = (r: Record<string, unknown>): ContentRecord => ({
  ...
  status: String(r.status) as ContentStatus, publishedAt: r.published_at ? String(r.published_at) : undefined, createdAt: String(r.created_at), updatedAt: String(r.updated_at), authorName: String(r.author_name ?? 'Arharif'),
});
