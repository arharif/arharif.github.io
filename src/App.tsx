  ...
  return (
    <section className="relative"> 
      ...
      <motion.section aria-label="Explore X1" className="landing-portals grid gap-5 py-4 md:grid-cols-2" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: 'easeOut' }}>
        <button className="landing-portal glass" onClick={() => nav('/x1-academic')}><span className="landing-portal-icon"><GraduationCap aria-hidden="true" /></span><span className="academic-eyebrow">Knowledge portal</span><strong>X1 Academic</strong><span>Structured cybersecurity knowledge, framework guidance and professional learning resources.</span><span className="landing-portal-link">Explore Academic <span aria-hidden="true">→</span></span></button>
        <button className="landing-portal glass" onClick={() => nav('/insights-innovation')}><span className="landing-portal-icon"><Lightbulb aria-hidden="true" /></span><span className="academic-eyebrow">Knowledge &amp; Exploration</span><strong>Insights &amp; Innovation</strong><span>Exploring technology, ideas, philosophy, human behaviour and emerging trends—connecting curiosity, critical thinking and different perspectives to better understand how the world is changing.</span><span className="landing-portal-link">Explore Ideas <span aria-hidden="true">→</span></span></button>
      </motion.section>
    </section>
  );
}
